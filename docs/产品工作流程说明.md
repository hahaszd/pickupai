...
现在主要通过**自助注册**流程完成。Tradie 访问 `https://www.getpickupai.com.au/dashboard/signup`，填写公司信息、邮箱和密码，然后通过 Stripe 完成付款。付款成功后，系统自动购买一个澳洲座机号码并分配给该用户。

管理员也可以通过 Admin API 手动创建账户：
...

{
  ...
  "ai_name": "Olivia",
  "twilio_number": "+61268000835",   ← 分配给该 Tradie 的 Twilio 号码
  "owner_phone": "+61412000000",    ← Tradie 的个人手机（接收短信）
  "owner_email": "mike@example.com", ← 登录邮箱
  "password": "secure-password",     ← 登录密码
...
- 在自助注册流程中，付款后自动购买澳洲座机号码并通过短信通知用户
...
OpenAI Realtime API（模型：`gpt-realtime-1.5`）负责以下所有环节：
- **语音转文字**（实时转录来电者说话内容）
...
**后台地址：** `https://getpickupai.com.au/dashboard/login`
...
tenants（租户表）
  └── tenant_id（主键）
  └── name（公司名）, ai_name（AI名称）, twilio_number（Twilio号）, owner_phone（手机号）
  └── owner_email（登录邮箱）, trade_type（服务类型）, service_area（服务区域）
  └── password_hash（密码哈希）, session_token（会话令牌）
  └── payment_status（付款状态）, stripe_customer_id, stripe_subscription_id
  └── provision_status（号码购买状态）, provision_error（购买失败原因）
...
tenant_sms_log（租户短信日志表）
  └── sms_id（主键）, tenant_id（外键 → tenants）
  └── to_phone（目标号码）, body（短信内容）, status, twilio_sid
  └── sent_at（发送时间）
```
...
| Admin API 保护 | 所有 `/admin/*` 路由需携带 `x-admin-token` 请求头或 `?token=` 查询参数（值为 `ADMIN_TOKEN`） |
...
| **AI 语音引擎** | OpenAI Realtime API（`gpt-realtime-1.5`） |
| **后端服务** | Node.js + Express + TypeScript |
| **WebSocket 桥接** | `ws` 包 |
| **数据库** | SQLite（通过 `sql.js` WASM 方式，无需原生依赖） |
| **短信发送** | Twilio Programmable Messaging（通过 Messaging Service + 字母数字发件人 ID） |
| **支付** | Stripe Checkout（订阅制，14 天免费试用） |
| **管理后台** | 服务端渲染 HTML（内置于 Express） |
| **部署方式** | Docker / Railway —— 已上线：`https://www.getpickupai.com.au` |
| **本地开发隧道** | ngrok |
...
- **生产环境**：以 Docker 容器形式部署在 Railway 上 —— 当前已上线：`https://www.getpickupai.com.au`
...
OWNER_PHONE_NUMBER=+61...           # 管理员手机号（接收系统告警）
TWILIO_MESSAGING_SERVICE_SID=MG...  # Messaging Service（字母数字发件人 ID）
TWILIO_ADDRESS_SID=AD...            # 澳洲号码购买所需的地址 SID
TWILIO_BUNDLE_SID=BU...             # 澳洲本地号码合规 Bundle SID（2026年6月起必需）
